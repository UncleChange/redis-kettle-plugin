 ...
 */
@Step(id = "RedisReset", image = "ref.png", name = "Redis增量缓存重置",
        description = "在每一次抽取之前增量缓存状态重置 必须与Redis增量缓存在不同的transform中", categoryDescription = "应用")
public class RedisResetMeta extends BaseStepMeta implements StepMetaInterface {
    ...
    public Object clone() {
        RedisResetMeta retval = (RedisResetMeta) super.clone();
    ...
    public StepInterface getStep(StepMeta stepMeta, StepDataInterface stepDataInterface, int cnr, TransMeta tr,
                                 Trans trans) {
        return new RedisReset(stepMeta, stepDataInterface, cnr, tr, trans);
    }

    public StepDataInterface getStepData() {
        return new RedisResetData();
    }
    ...
    public void check(List<CheckResultInterface> remarks,
        ...
        if (prev == null || prev.size() == 0) {
            cr =
                    new CheckResult(CheckResultInterface.TYPE_RESULT_WARNING, BaseMessages.getString(PKG,
                            "RedisResetMeta.CheckResult.NotReceivingFields"), stepMeta);
            remarks.add(cr);
        } else {
            cr =
                    new CheckResult(CheckResultInterface.TYPE_RESULT_OK, BaseMessages.getString(PKG,
                            "RedisResetMeta.CheckResult.StepRecevingData", prev.size() + ""), stepMeta);
            remarks.add(cr);
        }
        ...
        if (input.length > 0) {
            cr =
                    new CheckResult(CheckResultInterface.TYPE_RESULT_OK, BaseMessages.getString(PKG,
                            "RedisResetMeta.CheckResult.StepRecevingData2"), stepMeta);
            remarks.add(cr);
        } else {
            cr =
                    new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, BaseMessages.getString(PKG,
                            "RedisResetMeta.CheckResult.NoInputReceivedFromOtherSteps"), stepMeta);
            remarks.add(cr);
        }
    }
    ...
    private void readData(Node stepnode) throws KettleXMLException {
        try {
            ...
            throw new KettleXMLException(BaseMessages.getString(PKG, "RedisResetMeta.Exception.UnableToReadStepInfo"),
                    e);
        }
    }
    ...
    public void readRep(Repository rep, ObjectId id_step, List<DatabaseMeta> databases, Map<String, Counter> counters)
        ...
        try {
            ...
            throw new KettleException(BaseMessages.getString(PKG,
                    "RedisResetMeta.Exception.UnexpectedErrorReadingStepInfo"), e);
        }
    }
    public StepDialogInterface getDialog(Shell shell, StepMetaInterface meta, TransMeta transMeta, String name) {
        return new RedisResetDialog(shell, meta, transMeta, name);
    }
    public void saveRep(Repository rep, ObjectId id_transformation, ObjectId id_step)
        ...
        try {
            ...
            throw new KettleException(BaseMessages.getString(PKG,
                    "RedisResetMeta.Exception.UnexpectedErrorSavingStepInfo"), e);
        }
    }
